                            ...
                            <ul className="join-list">
                                ...
                                <li className="join-list-box">
                                    ...
                                    <div className="col-3-wrap">
                                        <div class="col-3"><SelectBasic type={"year"} state={"error"} helperText={"만 14세 이상, 1900년대 이후만 가능합니다."}/></div>
                                        <div class="col-3"><SelectBasic type={"month"} state={"error"}/></div>
                                        <div class="col-3"><SelectBasic type={"day"} state={"error"}/></div>
                                    </div>
                                    <span className="helperText">만 14세 이상, 1900년대 이후만 가능합니다.</span>
                                </li>
                            </ul>
                                ...
                                <div>
                                    ...
                                    {data?.map((data, key) => (
                                        <div key={key} className="check-list-wrap">
                                            <div>
                                            <Checkbox type='checkbox' name={`select-${data.id}`}
                                                onChange={(e) => handleSingleCheck(e.target.checked, data.id)}
                                                checked={checkItems.includes(data.id) ? true : false} />
                                            </div>
                                            {
                                                data.id == 1 ?
                                                <div className='second-row'><h4><span onClick={handleClickOpen}>이용약관</span>동의</h4></div> :
                                                data.id == 2 ?
                                                <div className='second-row'><h4><span onClick={handleClickOpen2}>개인정보처리방침</span>동의</h4></div> :
                                                <div className='second-row'><h4>{data.title}</h4></div>
                                            }
                                        </div>
                                        ))}
                                <BasicModal handleClose={handleClose} open={open} title={"이용약관"} content={"이용약관"}/>
                                <BasicModal handleClose={handleClose2} open={open2} title={"개인정보처리방침"} content={"개인정보처리방침"}/>
                            </div>
                        </JoinWrap>
                    </MobileContainerBox>
